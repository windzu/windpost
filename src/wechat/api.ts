...
export class WechatApiClient {
  ...
  async uploadArticleImage(file: MultipartFile): Promise<string> {
  ...
  async uploadPermanentImage(file: MultipartFile): Promise<string> {
...
function buildMultipart(file: MultipartFile): { boundary: string; body: ArrayBuffer } {
...
function throwWechatError(data: WechatError, status: number): void {
  if (!data.errcode && status < 400) return;
  const code = data.errcode ?? status;
  const hints: Record<number, string> = {
    40001: "AppSecret 无效，请检查公众号配置。",
    40125: "AppSecret 无效，请检查公众号配置。",
    40164: "当前公网 IP 不在公众号接口白名单中。",
    45009: "微信接口调用已达到频率上限。",
    48001: "当前公众号没有草稿接口权限。",
  };
  throw new Error(hints[code] || `微信接口失败（${code}）：${data.errmsg || "未知错误"}`);
}
